 ...
 */
  ...
  Float_t weightedMeanAngle[9];             //JOSCHA: [9]: one "normal" version and one for each channel ignored
  ...
  //JOSCHA:
  tree->Branch("weightedMeanAngle", weightedMeanAngle, "weightedMeanAngle[9]/F");
 ...
 */
        ...
        //Override the previous decision; after discussion, it was decided that the baseline before the peak is more reliable
        BL_used[i] = BL_lower[i];
 ...
 */
      ...
      //ADDED BY JOSCHA (new angles):
        //This calculates the weighted mean angle of photon impact for each event.
      float angleList[] = { 135.0,180.0,225.0,270.0,90.0,45.0,0.0,315.0 }; //Represents channels 0-7
      float cartX, cartY; //Cartesian coordinates
      float cartXSum;
      float cartYSum;
      int nPhotons;

      //To calculate a mean angle for an event, convert the polar vector (phi, r) -- where phi is 0,45,... and r is the number of Photons for one event in the corresponding channel -- to cartesian coordinates.
      //Then, add up all cartesian coordinates to get a "mean vector", and then convert back to polar coordinates. The phi of the mean vector is the mean angle.
      //We can assume that 1 photon corresponds to r = 1.

      //This is done 9 times: Once with all channels included and once for each of the 8 channels (only WOM D) ignored.      

      for (int i = 0; i < 9; i++)   //Loop over different channels ignored; i=0-7 --> ignore respective channel; i=8 --> All channels
      {
          cartXSum = 0;
          cartYSum = 0;
          nPhotons = 0;
          for (int j = 0; j < 8; j++) //only for WOM D
          {
              if (j != i)    //Ignore channel j
              {
                  //Convert into cartesian coordinates and add them up:
                  cartX = TMath::Cos(angleList[j] * (TMath::Pi() / 180.0)) * Integral[j];
                  cartY = TMath::Sin(angleList[j] * (TMath::Pi() / 180.0)) * Integral[j];
                  cartXSum += cartX;
                  cartYSum += cartY;
                  nPhotons += 1;
              }
          }
          if (cartXSum > 0 && cartYSum > 0)
          {
              weightedMeanAngle[i] = TMath::ATan(cartYSum / cartXSum) * 180.0 / TMath::Pi();
          }
          else if (cartXSum < 0)
          {
              weightedMeanAngle[i] = (TMath::ATan(cartYSum / cartXSum) + TMath::Pi()) * 180.0 / TMath::Pi();
          }
          else if (cartXSum > 0 && cartYSum < 0)
          {
              weightedMeanAngle[i] = (TMath::ATan(cartYSum / cartXSum) + 2 * TMath::Pi()) * 180.0 / TMath::Pi();
          }
          else if (cartXSum == 0 && cartYSum > 0)
          {
              weightedMeanAngle[i] = 90.0;
          }
          else if (cartXSum == 0 && cartYSum < 0)
          {
              weightedMeanAngle[i] = 270.0;
          }
          else if (cartXSum > 0 && cartYSum == 0)
          {
              weightedMeanAngle[i] = 0.0;
          }
          //Adjusting the histograms such that the peak is always at the center:
          if ((i == 0 && weightedMeanAngle[i] > 135.0) || (i == 1 && weightedMeanAngle[i] > 180.0) || (i == 2 && weightedMeanAngle[i] > 225.0) || (i == 3 && weightedMeanAngle[i] > 270.0) ||
              (i == 4 && weightedMeanAngle[i] > 90.0) || (i == 5 && weightedMeanAngle[i] > 45.0) || (i == 6 && weightedMeanAngle[i] > 360.0) || (i == 7 && weightedMeanAngle[i] > 315.0) ||
              (i == 8 && weightedMeanAngle[i] > 180.0))
          {
              weightedMeanAngle[i] -= 360.0;
          }
      }
      //END OF JOSCHA'S PART
